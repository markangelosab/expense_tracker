<template>
    <div class="flex justify-center items-center max-h-screen">
      <div class="max-w-9xl mx-auto p-4 bg-white rounded-lg shadow-lg border border-gray-300">
        <h1 class="text-4xl text-center text-gray-800 mb-5 bold">Income Tracker</h1>
        <div class="flex">
                <div class="flex-1 mr-5 max-w-xs">
                    <input type="text" v-model="newExpense.item" placeholder="Source of Fund" class="input-field bold">
                    ...
                    <button @click="addOrUpdateExpense" class="btn-primary bold">{{ editingIndex === null ? 'Add New Income' : 'Update Income' }}</button>
                </div>
          <div class="flex-1 relative"> 
              ...
              <table class="w-full">
                <thead>
                  <tr>
                    ...
                    <th class="border border-gray-300 px-4 py-2">Source of Fund</th>
            ...
            <div class="total-expenses border-t border-gray-300 bold text-green-500">Total Income: ₱{{parseFloat( totalExpenses ).toFixed(2)}}</div>
          </div>
        </div>
      </div>
    </div>
...
const fetchExpenses = async () => {
    const q = query(collection(db, 'Expense_tracker'), orderBy('dateTime', 'asc'));
...
const confirmDelete = (id) => {
    const result = window.prompt('Type "DELETE" to confirm deletion:');
    if (result === 'DELETE') {
        deleteExpense(id);
        window.alert('Deletion successful!');
    } else {
      window.alert('Deletion unsuccessful due to wrong input!');
    }
};
